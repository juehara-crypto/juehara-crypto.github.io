...
|過去シリーズで扱った問題|Ansible×Terraform連携での発現文脈|
|---|---|
|shellモジュール・state: latestによる冪等性崩壊（冪等性シリーズ第 **[1](https://juehara-crypto.github.io/blog/infra/ansible/ansible-idempotency/ansible-idempotency-01/)** ・**[4](https://juehara-crypto.github.io/blog/infra/ansible/ansible-idempotency/ansible-idempotency-04/)** 回）|local-exec経由でAnsibleを複数回呼び出す構成で、冪等でないタスクが累積して実行される|
|手動変更・Ansible管理外の変化によるドリフト（ドリフトシリーズ第 **[0](https://juehara-crypto.github.io/blog/infra/ansible/ansible-drift/ansible-drift-00/)**・**[1](https://juehara-crypto.github.io/blog/infra/ansible/ansible-drift/ansible-drift-01/)**・**[3](https://juehara-crypto.github.io/blog/infra/ansible/ansible-drift/ansible-drift-03/)** 回）|terraform applyによるリソース再生成後にAnsible設定が消失する・二重管理による上書きでドリフトが発生する|
|--checkで検知できないドリフト（**[ドリフトシリーズ第2回](https://juehara-crypto.github.io/blog/infra/ansible/ansible-drift/ansible-drift-02/)**）|AnsibleとTerraformの管理範囲が重複する領域では、どちらのツールでも状態の全体像を把握できない|
...
**次回：第2回：Terraform完了直後のプロビジョニング失敗を防ぐSSH待機制御**

---

📑 連載の移動　**前の記事：[【Moleculeシリーズ】第4回](https://juehara-crypto.github.io/blog/infra/ansible/ansible-molecule/ansible-molecule-04/)　｜　次の記事：【Ansible×Terraform編】第2回**
...
| 第3回     | 動的インベントリ生成時における出力データのパースエラー        | TerraformのJSON出力とAnsibleが期待する動的インベントリのJSONスキーマとの構造的な差異を整理し、生のTerraform出力をそのまま渡した場合に発生する「静かな失敗」を実機で再現したうえで、変換スクリプトによる解決方法を解説する。                                                                                            |
| 第4回 | 自動生成されたSSH鍵のパーミッション設定エラー           | Terraformで自動生成した秘密鍵ファイルの権限設定が不適切なため、AnsibleのSSH実行時に接続を拒否されるトラブルへの対応。                                                                                                                                                      |
| 第5回     | 仮想環境におけるIPアドレス変動対策                 | Terraformがリソースを再生成する際に発生するIPアドレス変動を実機で検証する。`terraform apply`単体ではtfstateとインベントリが自動的に追従する一方、applyとAnsibleの実行タイミングが分離すると、SSH接続自体は成功するのに意図しないホストへ接続してしまう危険があることを示す。解決策として、TerraformリソースレベルでのIP固定と動的インベントリという2つのアプローチを実機で比較する。 |
| 第6回     | ネットワーク初期化完了前に発生する接続タイムアウト          | Terraformでのネットワーク構築完了直後にAnsibleから接続しようとすると、ネットワークの反映待ちによってSSH接続がタイムアウトする問題を扱う。この問題は、DockerネットワークのブリッジやAWSのSecurity Group・VPCルーターの伝播ラグなど、実装方式が異なっていても「ネットワークの構築完了」と「実際に通信可能な状態」が別のタイミングであるという共通の構造から生じることを整理し、対策設計を解説する。   |
...
| 第8回     | 複数インスタンス同時構築時における並列処理の競合           | Terraformで複数リソースを同時生成する際、Ansible側の並列実行数（forks）の制限によって処理が遅延・競合する可能性がある問題を扱う。同時生成するリソース数が実行環境のキャパシティに対して相対的に多い場合に起こりうる構造的な問題であることを整理し、forks調整やバッチ分割といった対策設計を解説する。                                                           |
